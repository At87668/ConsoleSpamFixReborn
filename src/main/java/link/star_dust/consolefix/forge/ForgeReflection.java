...
/**
 * Reflection helpers for accessing Minecraft / Forge internals.
 *
 * <p>Forge 1.18+ uses Mojang names at runtime, so class and method names are
 * used directly. Event registration uses the 4-arg {@code addListener} overload
 * to avoid the generic-signature resolution problem of dynamic proxies.
 */
final class ForgeReflection {
    ...
    /**
     * Load a class visible to the Forge mod-loading environment. Tries the
     * calling classloader, the thread context classloader, and the classloader
    ...
    static Class<?> resolveTextComponentClass() {
        ...
        cachedTextClass = forName(ForgeReflectionConstants.CLS_COMPONENT);
    ...
    static Object createText(String message) {
        ...
        Object r = callStatic(ForgeReflectionConstants.CLS_COMPONENT,
                ForgeReflectionConstants.M_COMPONENT_LITERAL,
    ...
    private static Method findMethodImpl(Class<?> cls, String name, Class<?>[] paramTypes) {
        ...
        // Parameter-type scan across the class hierarchy.
    ...
    /** Resolve the Forge config directory ({@code FMLPaths.CONFIGDIR}). */
     ...
     * Class, Consumer)} overload so Forge does not need to resolve the event
     * type from the consumer's generic signature.
     */
